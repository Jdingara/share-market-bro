...
**CALL is gated per model, not with one flat switch (since 2026-08-12).** The **primary (15-min) model stays PUT-only by default** - its CALL confidence is still flat/unreliable even after adding VIX + Greeks features. Add `--allow-calls` to take its CALL signals anyway. The **early-session (5-min) model now allows CALL by default** - its CALL calibration turned out to be genuinely good (62%->80% precision climbing cleanly with confidence, see `PROJECT_STATUS.md`). Add `--block-calls-early-session` to be extra-conservative and block CALL there too. **Both are also available on the dashboard** ("Allow CALL trades (primary 15-min model)" and "Block CALL trades on early-session model too (5-min)" checkboxes).
