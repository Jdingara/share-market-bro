...
**No new entries before 12:30 PM by default (added 2026-08-13).** Analyzing all real paper trades logged so far found 10:30-12:30 (entirely the early-session model's window) lost Rs 5,889.65 net across 32 trades, while every window from 12:30 onward was net positive - simulating the cutoff on the real trade history turns +Rs 6,353 actual into +Rs 12,243. Add `--earliest-entry-time HH:MM` to change it (e.g. `--earliest-entry-time 00:00` to deliberately gather more early-morning data). **Also available on the dashboard** ("No new entries before" time picker).
...
**CALL is gated per model, not with one flat switch (since 2026-08-12), but both stay PUT-only by default (since 2026-08-15).** The early-session model briefly defaulted to CALL-allowed (08-12 to 08-14) based on a threshold scan that turned out to be measuring a stale backtester stop-loss (-10% instead of the live -5%) - reverted once corrected, since neither model's CALL precision actually climbs with confidence under the real bracket. Add `--allow-calls` (primary model) or `--allow-calls-early-session` (early-session model) to take CALL signals anyway - independent flags, use either or both deliberately, e.g. to gather more CALL data. **Both are also available on the dashboard** ("Allow CALL trades (primary 15-min model)" and "Allow CALL trades (early-session 5-min model)" checkboxes, both unchecked by default).
